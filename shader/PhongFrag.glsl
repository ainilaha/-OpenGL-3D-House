...
vec3 Heidrich_Seidel(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir);
vec3 Cartoon(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir);
...
void main(){
    ...
    for(int i = 0; i < NR_POINT_LIGHTS; i++)
    {
        ...
        }else if(light_model_flag==4)
        {
            point_light_result += Heidrich_Seidel(pointLights[i], norm, FragPos, viewDir);
        }else if(light_model_flag==5)
        {
            point_light_result += Cartoon(pointLights[i], norm, FragPos, viewDir);
        }
        else{
    ...
    if(draw_norm_flag==1)
        FragColor = vec4(0.5,0.7,0.2,9.0);
}
...
vec3 wardIsotropic(vec3 n, vec3 v, vec3 l) {
    ...
    return vec3(dot(n, l) * (material.diffuse + Specular*material.specular));
}
...
uniform float AnisoRoughness = 2.0f;
uniform float Roughness = 1.5f;
vec3 SpecDirection = vec3(1,2,3);
/**********************************************
*Heidrich-Seidel  start
*
***********************************************/
vec3 Heidrich_Seidel(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
     vec3 t = cross(normal,normalize(SpecDirection));
     vec3 h = normalize(light.position+viewDir);
    ...
    // specular shading
    vec3 reflectDir = reflect(-lightDir, normal);
    float spec = pow(max(dot(normal,h), 0.0), 1.0/Roughness);
    // attenuation
    float distance = length(light.position - fragPos);
    float attenuation = 1.0 / (light.constant + light.linear * distance + light.quadratic * (distance * distance));
    ...
    vec3 ambient = light.ambient * material.ambient;
    vec3 diffuse = light.diffuse * diff * material.diffuse;;
    vec3 specular = light.specular * spec * material.specular*aniso;
    ambient *= attenuation;
    diffuse *= attenuation;
    specular *= attenuation;
    return (ambient + diffuse + specular);
...
vec3 Cartoon(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    vec3 discrete_color = vec3(0,0,0);
    vec3 vertex_color = material.diffuse+material.ambient;
    if (diff > 0.98) {
        discrete_color = vec3(1, 1, 1);
    } else if (diff > 0.95) {
    ...
    return discrete_color*(ambient + diffuse + specular);
}
/**********************************************
*cartoon end
*
